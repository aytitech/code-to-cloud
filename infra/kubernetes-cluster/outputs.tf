...
output "kubeconfig_command" {
  ...
  value       = "ssh root@${hcloud_server.k8s_node.ipv4_address} 'cat /etc/rancher/k3s/k3s.yaml' | sed 's/127.0.0.1/${hcloud_server.k8s_node.ipv4_address}/g' > ~/.kube/k8s-cloudmart.yaml"
}
